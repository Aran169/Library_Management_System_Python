...
def issued_b():
        ...
        if book_id.get() != '':
            ...
            if c.fetchall() :
                ...
                dt="-".join(dt)
                c.execute(f"insert into ss_ret values('{a1}','{b1}','{dt}',NULL,'pending');")
                c.execute(f"update ab set reg_no='{a1}' where book_id='{b1}';")
                mydb.commit()
                ms.showinfo('book issued successfully')
            else:
...
a=Button(root,text="ISSUE",height= 2, width=9,command=issued_b)
a.pack()
a.place(x=350, y=400)
root.mainloop()
